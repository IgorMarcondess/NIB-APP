...
export default function Usuario() {
  ...
  useEffect(() => {
    ...
    if (user?.cpfUser) {
      fetchPontuacoesCpf(); // ✅ chamada correta
      fetchPontuacoes(); // ✅ também chamada
    }
  }, [user?.cpfUser]);
  ...
  return (
    <SafeAreaView className="flex-1 bg-white px-4 justify-center items-center gap-4">
      <View className="items-center mt-4">
        <Text className="text-blue-700 font-bold text-2xl mt-6 mb-2">
          SUA PONTUAÇÃO
        </Text>
      ...
      <View className=" bg-[#dee2e6] w-96 h-[38rem] rounded-xl p-5 gap-2 ">
        <View className="items-center">
          <Text className="text-blue-700 font-extrabold text-xl mb-2">
            RANKING
          </Text>
        </View>
        <ScrollView
                  ...
                  <View className="flex-row items-center space-x-3 gap-4">
                    ...
                    <View className="rounded-full items-center justify-center">
                      <Image
                        ...
                        className="font-bold w-17 h-18"
                      />
                    </View>
                  </View>

                  <View className="items-center">
                    <Text className="text-lm font-bold">
                      {user.nomeUser.toUpperCase()}
                    </Text>
