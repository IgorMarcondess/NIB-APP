...
export default function Avaliacao() {
  ...
  const envioHabitos = async () => {
    ...
    if (!escovacao) {
      Alert.alert(
        "Atenção 🪥",
        "Você precisa marcar pelo menos a escovação dos dentes para continuar."
      );
      return;
    }
    ...
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View className="flex-1 justify-center items-center border-2 border-primary rounded-lg m-2 p-2">
          <Text className="text-center text-blue-700 font-bold text-2xl mb-4">
            O QUE FOI REALIZADO?
          </Text>
          ...
          >
            <Text
              className={`text-blue-700 text-xl font-extrabold ${
                fioDental ? "opacity-100" : "opacity-50"
              }`}
            >
              FIO DENTAL
            </Text>
          ...
          >
            <Text
              className={`text-blue-700 text-xl font-extrabold ${
                bochecho ? "opacity-100" : "opacity-50"
              }`}
            >
              BOCHECHO
            </Text>
          ...
          <TouchableOpacity
            className="bg-blue-700 rounded-full py-3 w-36 mt-4"
